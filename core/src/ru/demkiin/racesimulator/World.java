...
public class World {
        ...
        for (int y=0; y < map.getMap().size; y++) {
            for (int x=0; x < map.getMap().get(y).length(); x++) {
                ...
                if (map.getTile(x, y).equals("G")) {
                    // Generate girls ^^
                    girls.add(new Girl(x, map.getMap().size-y, 0.5f, 0.5f));
                }
                ...
                if (map.getTile(x, y).equals("V")) {
                    // Generate keys
                    enemys.add(new Enemy(this,x, map.getMap().size-y, 0.5f, 0.5f,0.5f+enemys.size*0.5f));
    ...
    public void CalculatePosition(){
        ...
        for(int i=0; i < enemys.size; i++ ) {
            if (enemys.get(i).centrePos.y>player.getCentrePos().y )
                tempPositon++;

        }

        playerPosition = tempPositon;
    }
    ...
    public boolean collision() {
        ...
        for(int i=0; i < girls.size; i++ ) {
            if ((girls.get(i).bounds.contains(player.getHitboxFrontRight().x, player.getHitboxFrontRight().y)) || (girls.get(i).bounds.contains(player.getHitboxBackRight().x, player.getHitboxBackRight().y)) || (girls.get(i).bounds.contains(player.getHitboxBackLeft().x, player.getHitboxBackLeft().y)) || (girls.get(i).bounds.contains(player.getHitboxFrontLeft().x, player.getHitboxFrontLeft().y))) {
    ...
    private void handleInput(float delta) {
        ...
        if (Gdx.app.getType() == ApplicationType.Android) {
            ...
            if (Gdx.input.isTouched()) {
                if (Gdx.input.getX() < RaceSimulator.WIDTH / 2) player.turnLeft(delta);
                if (Gdx.input.getX() > RaceSimulator.WIDTH / 2) player.turnRight(delta);
            }
        }
    ...
    private void EnemyMove(float delta){
        for(int i=0; i < getEnemys().size; i++ ) {
            getEnemys().get(i).moveForward(delta);
        }
